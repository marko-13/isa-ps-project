...
public interface DoctorRepository extends JpaRepository<Doctor, Long> {
    ...
    @Query(
            value = "SELECT ap.type, ap.id, ap.confirmed, ap.adress, ap.city, ap.deleted, ap.email, ap.enabled, ap.last_name, ap.last_password_reset_date, ap.enabled_patient, " +
    ...
    @Query(
            value = "SELECT ap.type, ap.confirmed, ap.id, ap.adress, ap.city, ap.deleted, ap.email, ap.enabled, ap.last_name, ap.last_password_reset_date, ap.enabled_patient, " +
                    "ap.mobile, ap.name, ap.password, ap.rejected, ap.state, ap.user_role, ap.review, ap.review_count, ap.shift, ap.jmbg," +
                    "ap.clinic_id, ap.clinical_center_id" +
    ...
    @Query(
            value = "SELECT ap.type, ap.id, ap.confirmed, ap.adress, ap.city, ap.deleted, ap.email, ap.enabled, ap.last_name, ap.last_password_reset_date," +
                    " ap.enabled_patient, " +
                    "ap.mobile, ap.name, ap.password, ap.rejected, ap.state, ap.user_role, ap.review, ap.review_count, ap.shift, ap.jmbg," +
                    "ap.clinic_id, ap.clinical_center_id" +
